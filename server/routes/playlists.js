...
router.get('/mine/list', requireUser, async (req, res) => {
  res.json({ playlists: await getUserPlaylists(req.user.id) });
});
...
router.post('/', requireUser, async (req, res) => {
  ...
  const pl = await createPlaylist(req.user.id, { title: title.trim(), description: description?.trim() || '' });
...
router.put('/:id', requireUser, async (req, res) => {
  ...
  const result = await updatePlaylist(req.params.id, req.user.id, { title, description, isPublic });
...
router.post('/:id/tracks', requireUser, async (req, res) => {
  ...
  const result = await addTrackToPlaylist(req.params.id, req.user.id, trackId);
  if (!result) return res.status(404).json({ error: 'Playlist or track not found' });
  res.json(result);
});

// 需登录：移除曲目
router.delete('/:id/tracks/:trackId', requireUser, async (req, res) => {
  const ok = await removeTrackFromPlaylist(req.params.id, req.user.id, req.params.trackId);
