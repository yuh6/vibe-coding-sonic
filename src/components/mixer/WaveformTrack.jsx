...
export default function WaveformTrack({ track, time, duration, loop, onSeek, onLoopChange }) {
  ...
  const draw = useCallback(() => {
    ...
    const barW = usableWidth / peaks.length;
    const mid = height / 2;
    ...
    for (let i = 0; i < peaks.length; i++) {
      ...
      ctx.fillStyle = x < playedX ? track.color : `${track.color}55`;
  ...
  const onPointerDown = (e) => {
    ...
    const move = (ev) => {
      ...
      if (Math.abs(cur - dragRef.current.start) > 0.2) {
        dragRef.current.moved = true;
        onLoopChange({
          start: Math.min(dragRef.current.start, cur),
          end: Math.max(dragRef.current.start, cur),
        });
      }
    };
    const up = (ev) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <span
        className="w-16 shrink-0 truncate text-right font-mono text-[10px]"
        style={{ color: track.color }}
        title={track.name}
      >
        {track.name}
      </span>
      <div ref={wrapRef} className="min-w-0 flex-1 cursor-crosshair rounded-lg bg-led-panel">
