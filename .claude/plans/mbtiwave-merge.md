...
```
(两者当前都未安装;MBTIWAVE 原用 React19,本项目是 React18 —— framer-motion@11 / lucide
最新版均兼容 React18,用与项目一致的 React18。)

### 2. 新建组件 `src/components/MBTIWAVE.jsx`
- 把 HTML 里 `<script type="text/babel">` 中的 JS 原样搬进来,改成标准模块:
  ...
  - 去掉 `createRoot(...).render(<App/>)`,改为 `export default function MBTIWAVE() { ...原 App 函数体... }`
...
### 3. 新建样式 `src/styles/mbtiwave.css`
把 HTML `<style>` 里 MBTIWAVE 专有的 CSS 抽出来(body 除外):
...
在 `MBTIWAVE.jsx` 顶部 `import '../styles/mbtiwave.css';`。
(不改全局 body 样式,避免影响现有页面。)

### 4. Tailwind safelist(关键)
MBTIWAVE 的 `mbtiData` 把颜色类名(如 `from-emerald-500/20`、`border-emerald-500/30`、
...
### 5. 接入路由 `src/App.jsx`
- 顶部 `import MBTIWAVE from './components/MBTIWAVE';`
- 加 `const isMBTIWAVE = hash === '#/mbtiwave';`
- 渲染分支链最前面加:`isMBTIWAVE ? <MBTIWAVE /> :` (让它全屏接管,不套现有 header/grid)
  —— MBTIWAVE 自带 header/footer,应整页渲染,所以放在最外层 return 里单独判断:
  若 isMBTIWAVE,直接 `return <MBTIWAVE />;`(在现有 return 之前),最简洁。
- 在现有顶栏导航区加一个入口链接 `<a href="#/mbtiwave">🌊 MBTIWAVE</a>`,风格同其它 pad 链接。

### 6. 验证
- `npm run dev` 已在跑;改完热更新。
- 访问 `http://localhost:5173/#/mbtiwave` 逐个视图点一遍:
...
## 交付
新增 3 处:`src/components/MBTIWAVE.jsx`、`src/styles/mbtiwave.css`、tailwind safelist;
